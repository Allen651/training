...
public class WeixinApiController extends BaseController {
	...
	public AjaxJson downloadVoiceFromWx(String usercode, String mediaId, HttpServletRequest request,
			HttpServletResponse response) {
		String accessToken = getAccessToken();
		String fileSavePath = Global.getUserfilesBaseDir() + Global.USERFILES_BASE_URL + "voice/";
		FileOperUtils.createDirectory(fileSavePath);
		// String upLoadPath = getUpLoadPath(0);
		AjaxJson ajaxJson = new AjaxJson();
		if (StringUtils.isEmpty(mediaId)) {
		...
		try {
			...
			String weixinServerFileName = ContentDisposition.substring(ContentDisposition.indexOf("filename") + 10,
					ContentDisposition.length() - 1);
			weixinServerFileName = usercode + weixinServerFileName.substring(weixinServerFileName.indexOf("."));
			fileSavePath += weixinServerFileName;
			// 修改图片保存路径及名称
			FileUtils.copyInputStreamToFile(in, new File(fileSavePath));
			String imageUrl = Global.USERFILES_BASE_URL + "voice/" + weixinServerFileName;
			System.out.println("----------weixin download voice success msg---------");
	...
	public AjaxJson wxLogin(String openid, String role, HttpServletRequest request, HttpServletResponse httpResponse) {
		httpResponse.setHeader("Access-Control-Allow-Origin", "*");
		AjaxJson ajaxJson = new AjaxJson();
		...
		if (userApi != null) {
			...
			if ("Anchor".equals(role)) {
				AnchorInfo anchorInfo = anchorInfoService.findUniqueByProperty("usercode",
						ajaxJson.getBody().get("usercode"));
				if (anchorInfo == null) {
					// 报名页
					ajaxJson.setErrorCode("4");
					return ajaxJson;
				}
				String roomNo = anchorInfo.getPlayHomeid();
				String certification = anchorInfo.getCertification();
				if (anchorInfo != null && (StringUtils.isNotBlank(roomNo))) {
					// 主页
					ajaxJson.setErrorCode("1");
					return ajaxJson;
				}
				if (anchorInfo != null && !Global.PASS.equals(certification)) {
					// 实名认证页
					ajaxJson.put("name", anchorInfo.getName());
					ajaxJson.put("idcard", anchorInfo.getIdcard());
					ajaxJson.setErrorCode("3");
					return ajaxJson;
				}
				if (anchorInfo != null && (StringUtils.isBlank(roomNo))) {
					// 审核状态页
			...
			if ("Talent".equals(role)) {
				TalentInfo zhaomuTalent = talentInfoService.findBymobile(userApi.getMobile());
				if (zhaomuTalent == null) {
					// 报名页
					ajaxJson.setErrorCode("4");
					return ajaxJson;
				}
				String contract = zhaomuTalent.getContract();
				String certification = zhaomuTalent.getCertification();
				if (zhaomuTalent != null && Global.CONTRACT_COMPLETED.equals(contract)) {
					// 主页
					ajaxJson.setErrorCode("1");
					return ajaxJson;
				}
				if (zhaomuTalent != null && !Global.PASS.equals(certification)) {
					// 实名认证页
					ajaxJson.put("name", zhaomuTalent.getName());
					ajaxJson.put("idcard", zhaomuTalent.getIdcard());
					ajaxJson.setErrorCode("3");
					return ajaxJson;
				}
				if (zhaomuTalent != null && !Global.CONTRACT_COMPLETED.equals(contract)) {
	...
	@ResponseBody
	@RequestMapping(value = "getAnchorWxId")
	public AjaxJson getWxIdAPI(HttpServletRequest request, String role, HttpServletResponse httpResponse) {
		httpResponse.setHeader("Access-Control-Allow-Origin", "*");
		AjaxJson ajaxJson = getWxId(request, httpResponse);
		if (!ajaxJson.isSuccess()) {
			return ajaxJson;
		}
		AnchorInfo anchorInfo = anchorInfoService.findUniqueByProperty("usercode", ajaxJson.getBody().get("usercode"));
		if (anchorInfo == null) {
			// 报名页
			ajaxJson.setSuccess(true);
			ajaxJson.setErrorCode("4");
		...
		if (Global.NO.equals(anchorInfo.getIsUseable())) {
			ajaxJson.setSuccess(true);
			ajaxJson.setErrorCode("9");
			return ajaxJson;
		}
		String roomNo = anchorInfo.getPlayHomeid();
		String certification = anchorInfo.getCertification();
	...
	@ResponseBody
	@RequestMapping(value = "getTalentWxId")
	public AjaxJson getTalentWxId(HttpServletRequest request, HttpServletResponse httpResponse) {
		httpResponse.setHeader("Access-Control-Allow-Origin", "*");
		AjaxJson ajaxJson = getWxId(request, httpResponse);
		if (!ajaxJson.isSuccess()) {
			return ajaxJson;
		}
		TalentInfo zhaomuTalent = talentInfoService.findUniqueByProperty("usercode",
				ajaxJson.getBody().get("usercode"));
		if (zhaomuTalent == null) {
			// 报名页
			ajaxJson.setSuccess(true);
		...
		if (Global.NO.equals(zhaomuTalent.getIsUseable())) {
			ajaxJson.setSuccess(true);
			ajaxJson.setErrorCode("9");
			return ajaxJson;
		}
	...
	@ResponseBody
	@RequestMapping(value = "getWxShare")
	public AjaxJson getWxShare(HttpServletRequest request, HttpServletResponse httpResponse) {
		httpResponse.setHeader("Access-Control-Allow-Origin", "*");
		String tempid = request.getParameter("id");
		String usercode = request.getParameter("usercode");
		String type = request.getParameter("type");
		AjaxJson ajaxJson = new AjaxJson();
		if (!StringUtils.isBlank(tempid) && !StringUtils.isBlank(usercode) && !StringUtils.isBlank(type)) {
			TalentInfo talentinfo = talentInfoService.findByusercode(Integer.valueOf(usercode));
			String qcode_url = talentinfo.getQcodeUrl();// 招募主播二维码
			String talentQcode = talentinfo.getTalentQcodeUrl();// 招募星探二维码
			Propaganda pro = propagandaService.get(tempid);
			String share_img = pro.getImg();
			if (type.equals("talent")) {
				ajaxJson.put("qcode_url", talentQcode);
			} else {
				ajaxJson.put("qcode_url", qcode_url);
			}
			ajaxJson.put("share_img", share_img);
		} else {
			ajaxJson.setSuccess(false);
			ajaxJson.setMsg("参数不能为空！");
		}
		return ajaxJson;
	}
	...
	public AjaxJson bindWx(String mobile, String validCode, String openid, HttpServletResponse response) {
		response.setHeader("Access-Control-Allow-Origin", "*");
		AjaxJson ajaxJson = new AjaxJson();
		...
		if (StringUtils.isBlank(openid)) {
			ajaxJson.setSuccess(false);
			ajaxJson.setMsg("微信openid不能为空！");
			return ajaxJson;
		}
		Integer cacheCode = (Integer) CacheUtils.get(CacheUtils.USER_API_CACHE, mobile);
		if (Integer.parseInt(validCode) != cacheCode) {
			ajaxJson.setSuccess(false);
			ajaxJson.setMsg("验证码错误或已失效");
			return ajaxJson;
		}
		UserApi userRegisted = userApiService.findBymobile(mobile);
		if (userRegisted == null) {
			ajaxJson.setSuccess(false);
			ajaxJson.setMsg("绑定账户不存在,请先注册");
			ajaxJson.setErrorCode("1");
			return ajaxJson;
		} else {
			userRegisted.setWeixinOperid(openid);
			userApiService.save(userRegisted);
			String token = UUID.randomUUID().toString();
			// 将token及用户信息放入缓存
			CacheUtils.put(CacheUtils.USER_API_CACHE, token, userRegisted);
			ajaxJson.put("token", token);
			ajaxJson.put("usercode", userRegisted.getUsercode());
			ajaxJson.setSuccess(true);
	 ...
	 * @throws Exception
	 */
	public Response sendTemplate(String touser, String template_id, String url, Map data) throws Exception {
		String access_token = getAccessToken();
		String message_Template_url = Global.getConfig("message_Template_url");
		message_Template_url = message_Template_url.replace("ACCESS_TOKEN", access_token);
		...
		HttpResponse response = HttpUtils.doPost(message_Template_url, null, "Post", headers, querys,
				requestBody.toJSONString());
		res = HttpUtils.convert(response);
		return res;
	}
	...
	public AjaxJson getUnifiedorder(String orderCode, String openerId, int money, String desc, HttpServletRequest rep,
		...
		try {
			...
			// String
			// pay_callback_url="http://dev-admin.bbohu.com/f/api/weixin/payCallBack";
	...
	public AjaxJson getzhifu(String orderCode, String openerId, int money, String desc, HttpServletRequest rep,
			HttpServletResponse resp) {
		Map<String, String> paramsMapPay = null;
		AjaxJson ajaxJson = new AjaxJson();
		try {
			...
			String mch_id = Global.getConfig("MCHID");
			String apiSecret = Global.getConfig("APISECRET");
			String pay_callback_url = Global.getConfig("pay_zhifu_url");
			// String
			// pay_callback_url="http://dev-admin.bbohu.com/f/api/weixin/payCallBack";
			String nonce_str = Sign.create_nonce_str().replace("-", "");
			String ip = IpUtil.getIpAddr(rep);
	...
	private void zhifu(HttpServletRequest rep, HttpServletResponse resp) {
		...
		if (paySign.equals(payCallBack.getSign())) {
			if (payCallBack.getResult_code().equals("SUCCESS") && payCallBack.getReturn_code().equals("SUCCESS")) {
				...
				synchronized (paySign) {
					ResourceOrder res = resService.get(payCallBack.getOut_trade_no());
					res.setStatus("1");
					resService.save(res);
					UserApi userPuber = userApiService.get(res.getMessageId());
					...
					if (StringUtils.isNotBlank(topuber)) {
						...
						// 根据具体模板参数组装
						String title = "您的人气订单已经支付成功";
						String orderCode = res.getOrderCode().toString();
						String status = "已付款";
						String info = "订单金额：" + res.getPrice() + "元，登录系统查看订单详情";
						String color = "#ff0000";
					...
					Product product = proService.get(res.getTypeId());
					String[] str = product.getContact().split(",");
					List<String> list = new ArrayList<String>();
					for (String s : str) {
						list.add(userDao.findModel(s));
					}
					AnchorPersonModel puber = anchorInfoService.getBasicInfo(res.getMessageId());
					Bill bill = new Bill();
					bill.setname(puber.getName());
					bill.setUserId(userPuber.getId());
					bill.setType("人气订单");
					bill.setMoney(res.getPrice().doubleValue());
					bill.setDesc("人气订单，转账");
					bill.setBeginCreateDate(new Date());
					bill.setStatus(0);//未结算
					bill.setOrderId(res.getId());
					billService.save(bill);
					// 短信通知经纪人
					String contentJson = "{\"product\":\"" + "下单人："+puber.getName() + "金额："+res.getPrice() + "\"}";
					try {	
						for (String mobile : list) {
							SmsSender.sendSms(mobile, SmsSender.SMS_ANCHOR_PLACE_PRODUCT_TEMPLATE,
									contentJson);
						}
